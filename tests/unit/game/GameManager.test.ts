...
describe('GameManager', () => {
  ...
  describe('state transitions', () => {
    ...
    it('should transition from play_phase to score_screen when production target is reached', () => {
      // CONTRACT: Runs end only via auto-completion when outputsDelivered
      // reaches the level's required production count. There is no manual
      // end-of-run path.
      // GIVEN
      gm.startLevel('level_1')
      gm.startSimulation()
    ...
    it('should transition back to level_select from score_screen', () => {
      // GIVEN — reach score_screen via auto-completion
      gm.startLevel('level_1')
      gm.startSimulation()
      gm.simulation!.robotsProduced = 5
      gm.simulation!.outputsDelivered = 5
  ...
  describe('event emitter', () => {
    ...
    it('should fire scoreReady after auto-completion', () => {
      // GIVEN
      const handler = vi.fn()
